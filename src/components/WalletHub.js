  ...
  const handleSubmitProof = async () => {
    ...
    try {
      await apiClient.post('/wallet/payment-proof', {
        ...
        screenshotUrl: screenshotPreview // in a real app, upload file to s3 and send url
      });
      setIsSuccess(true);
    } catch (err) {
    ...
    <div className="p-4 bg-[#0B0E14] text-white min-h-[calc(100vh-80px)] font-mono space-y-4 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
        ...
        <div className="bg-[#161B22] p-4 rounded-xl border border-white/10">
          ...
          <span className="text-[10px] text-gray-500 mt-1 block">Earned invite commissions. <br/>(Manage in Referral tab)</span>
        </div>
      </div>
      ...
      <div className="bg-[#161B22] p-4 rounded-xl border border-white/10 flex flex-col min-h-[300px]">
        <h2 className="text-xs font-bold text-white mb-3 flex items-center gap-1.5 border-b border-[#3c494e]/30 pb-2">
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="text-gray-400 border-b border-[#3c494e]/20 text-[10px] bg-black/10">
                ...
                <th className="py-2 px-2">METHOD / UTR</th>
                <th className="py-2 px-2">STATUS</th>
                <th className="py-2 px-2">ADMIN NOTE</th>
                <th className="py-2 px-2 text-right">ACTION</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#3c494e]/10">
              {(!walletData.paymentHistory || walletData.paymentHistory.length === 0) ? (
                <tr><td colSpan="6" className="text-center py-10 text-gray-500">No payment requests found.</td></tr>
              ) : (
                walletData.paymentHistory.map((req, idx) => (
                  <tr key={idx} className="hover:bg-white/[0.01]">
                    <td className="py-2 px-2 text-gray-400 font-mono text-[10px]">{new Date(req.createdAt).toLocaleString()}</td>
                    <td className="py-2 px-2 font-extrabold text-white">₹{req.amount}</td>
                    <td className="py-2 px-2 text-gray-400">
                      <span className="text-[10px] bg-white/5 px-1.5 py-0.5 rounded mr-1">{req.method}</span>
                      {req.utr || 'N/A'}
                    </td>
                    <td className="py-2 px-2">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${
                        req.status === 'APPROVED' ? 'bg-[#00FF41]/10 text-[#00FF41] border-[#00FF41]/30' :
                        req.status === 'REJECTED' ? 'bg-[#ffb4ab]/10 text-[#ffb4ab] border-[#ffb4ab]/30' :
                        'bg-yellow-500/10 text-yellow-500 border-yellow-500/30'
                      }`}>
                        {req.status}
                      </span>
                    </td>
                    <td className="py-2 px-2 text-[10px] text-gray-500 max-w-[150px] truncate" title={req.adminNotes || 'None'}>
                      {req.adminNotes || '-'}
                    </td>
                    <td className="py-2 px-2 text-right">
                      {req.status === 'APPROVED' && (
                        <button 
                          ...
                          className="px-2.5 py-1 bg-[#00D4FF]/10 text-[#00D4FF] hover:bg-[#00D4FF]/20 rounded text-[10px] font-bold border border-[#00D4FF]/30 transition-colors"
                        >
                          RECEIPT
                        </button>
                      )}
      ...
      {/* Receipt Modal */}
      {selectedReceipt && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 font-mono text-xs">
          <div className="bg-[#10131a] border border-[#3c494e]/50 rounded-2xl w-full max-w-sm p-5 text-white relative shadow-2xl">
            <button 
            ...
            <div className="text-center border-b border-[#3c494e]/30 pb-4 mb-4">
              <div className="w-12 h-12 bg-[#00D4FF]/10 text-[#00D4FF] rounded-full flex items-center justify-center mx-auto mb-3 border border-[#00D4FF]/30">
                <CheckCircle2 className="w-6 h-6" />
              </div>
              <h2 className="text-sm font-extrabold text-white">PAYMENT SUCCESSFUL</h2>
              <p className="text-[#859398] text-[10px] mt-1">{new Date(selectedReceipt.updatedAt || selectedReceipt.createdAt).toLocaleString()}</p>
            </div>
            
            <div className="space-y-3">
              <div className="flex justify-between items-center bg-white/5 p-2 rounded">
                <span className="text-gray-400 text-[10px]">TRANSACTION ID</span>
                <span className="font-bold text-[#00D4FF]">{selectedReceipt.id}</span>
              </div>
              <div className="flex justify-between items-center bg-white/5 p-2 rounded">
                <span className="text-gray-400 text-[10px]">PAYMENT METHOD</span>
                <span className="font-bold">{selectedReceipt.method} ({selectedReceipt.utr || 'N/A'})</span>
              </div>
              <div className="flex justify-between items-center bg-white/5 p-2 rounded">
                <span className="text-gray-400 text-[10px]">BASE AMOUNT</span>
                <span className="font-bold">₹{selectedReceipt.amount}</span>
              </div>
              
              <div className="border-t border-[#3c494e]/30 pt-3 mt-3">
                <div className="flex justify-between items-center bg-[#00FF41]/5 border border-[#00FF41]/20 p-2 rounded">
                  <span className="text-[#00FF41] font-bold text-[10px]">TOKENS CREDITED</span>
                  <span className="font-extrabold text-[#00FF41] text-sm">{selectedReceipt.tokensCredited || selectedReceipt.amount}</span>
                </div>
              </div>
            </div>
            ...
            <button 
              onClick={() => setSelectedReceipt(null)}
              className="w-full mt-5 py-2.5 bg-white/10 hover:bg-white/20 text-white font-bold rounded-lg transition-colors"
            >
              CLOSE RECEIPT
            </button>
          </div>
