...
export default function OptionChain() {
  ...
  const filteredContracts = useMemo(() => {
    ...
    liveContracts.forEach((c, i) => {
      ...
      // If there is an exact tie (diff === minDiff), the lower strike (which comes first in the sorted array) naturally remains selected due to the < operator.
    });

    const atmStrike = liveContracts[atmIdx]?.strike;
